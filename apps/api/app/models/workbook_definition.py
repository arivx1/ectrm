...
class WorkbookDefinition(Base):
    __tablename__ = "workbook_definitions"
    __table_args__ = (
        UniqueConstraint(
            "workbook_key",
            "scope_owner_key",
            name="uq_workbook_definitions_key_owner",
        ),
    )

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    workbook_key: Mapped[str] = mapped_column(String(80), nullable=False, index=True)
