...
from apps.api.app.domains.reference_data.services.external_data.ercot_price_mapper import (
    normalize_ercot_price_observations,
)
    ...
    try:
        ...
        if not definitions and not price_mappings:
            raise ExternalSeriesSyncError(
                "No active ERCOT external series or price-index sources matched the requested filters"
            )
        ...
        price_observations = normalize_ercot_price_observations(
        ...
        if changed_price_index_codes:
            rebuild_trade_accruals_ledger(
                ...
                actor_id=requested_by or "external_data:ercot_sync",
                now=downloaded_at,
            )
...
def _load_definitions(
    ...
    *,
    ...
    stmt = select(ExternalSeriesDefinition).where(
        ExternalSeriesDefinition.provider == "ERCOT",
...
def _load_price_mappings(
    ...
    *,
    ...
    stmt = select(ReferencePriceIndexSource).where(
        ReferencePriceIndexSource.provider == "ERCOT",
