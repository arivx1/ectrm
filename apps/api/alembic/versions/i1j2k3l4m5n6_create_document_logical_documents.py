...
def upgrade() -> None:
    ...
    if not inspector.has_table("document_logical_documents"):
        op.create_table(
    ...
    indexes = {index["name"] for index in inspector.get_indexes("document_logical_documents")}
    if "ix_document_logical_documents_document_id" not in indexes:
        op.create_index("ix_document_logical_documents_document_id", "document_logical_documents", ["document_id"])
    if "ix_document_logical_documents_document_range" not in indexes:
        op.create_index(
            "ix_document_logical_documents_document_range",
            "document_logical_documents",
            ["document_id", "page_start", "page_end"],
        )
    if "ix_document_logical_documents_document_kind" not in indexes:
        op.create_index(
            "ix_document_logical_documents_document_kind",
            "document_logical_documents",
            ["document_kind"],
        )
    if "ix_document_logical_documents_review_status" not in indexes:
        op.create_index(
