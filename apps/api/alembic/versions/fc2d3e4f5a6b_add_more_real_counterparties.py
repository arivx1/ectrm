...
ADDITIONAL_REAL_COUNTERPARTY_DATA_PATH = (
    ...
    / "services"
    / "data"
    / "additional_real_counterparties.json"
)


def _load_counterparty_rows():
    with ADDITIONAL_REAL_COUNTERPARTY_DATA_PATH.open("r", encoding="utf-8") as handle:
        rows = json.load(handle)

    if len(rows) != ADDITIONAL_REAL_COUNTERPARTY_ROW_COUNT:
        raise RuntimeError(
            "Additional real counterparty migration count drifted unexpectedly: "
            f"expected {ADDITIONAL_REAL_COUNTERPARTY_ROW_COUNT}, got {len(rows)}"
        )

    return rows


ADDITIONAL_REAL_COUNTERPARTY_ROWS = _load_counterparty_rows()
...
def upgrade() -> None:
    for row in ADDITIONAL_REAL_COUNTERPARTY_ROWS:
        op.execute(
            f"""
...
def downgrade() -> None:
    for row in reversed(ADDITIONAL_REAL_COUNTERPARTY_ROWS):
        op.execute(
            f"""
