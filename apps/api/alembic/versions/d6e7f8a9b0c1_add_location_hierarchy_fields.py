"""add location hierarchy fields

Revision ID: d6e7f8a9b0c1
Revises: b4c5d6e7f8a9, d2c4b6a8e0f1
"""

from alembic import op
import sqlalchemy as sa

revision = "d6e7f8a9b0c1"
down_revision = ("b4c5d6e7f8a9", "d2c4b6a8e0f1")
branch_labels = None
depends_on = None


LOCATION_BACKFILLS = [
    {
        "code": "CUSHING",
        "parent_location_code": "PADD2",
        "location_kind": "POINT",
        "city": "Cushing",
        "state_or_territory": "Oklahoma",
        "continent": "North America",
        "latitude": 35.9853,
        "longitude": -96.7528,
    },
    {
        "code": "MIDLAND",
        "parent_location_code": None,
        "location_kind": "POINT",
        "city": "Midland",
        "state_or_territory": "Texas",
        "continent": "North America",
        "latitude": 31.9974,
        "longitude": -102.0779,
    },
    {
        "code": "HOUSTON_SHIP_CHANNEL",
        "parent_location_code": "USGC",
        "location_kind": "POINT",
        "city": "Houston",
        "state_or_territory": "Texas",
        "continent": "North America",
        "latitude": 29.7285,
        "longitude": -95.265,
    },
    {
        "code": "USGC",
        "parent_location_code": None,
        "location_kind": "REGION",
        "city": "New Orleans",
        "state_or_territory": "Louisiana",
        "continent": "North America",
        "latitude": 29.9511,
        "longitude": -90.0715,
    },
    {
        "code": "PADD2",
        "parent_location_code": None,
        "location_kind": "REGION",
        "city": "Des Moines",
        "state_or_territory": "Iowa",
        "continent": "North America",
        "latitude": 41.5868,
        "longitude": -93.625,
    },
    {
        "code": "NYH",
        "parent_location_code": None,
        "location_kind": "POINT",
        "city": "New York",
        "state_or_territory": "New York",
        "continent": "North America",
        "latitude": 40.684,
        "longitude": -74.0062,
    },
    {
        "code": "ARA",
        "parent_location_code": None,
        "location_kind": "REGION",
        "city": "Rotterdam",
        "state_or_territory": "South Holland",
        "continent": "Europe",
        "latitude": 51.9244,
        "longitude": 4.4777,
    },
    {
        "code": "HENRY_HUB",
        "parent_location_code": "USGC",
        "location_kind": "POINT",
        "city": "Erath",
        "state_or_territory": "Louisiana",
        "continent": "North America",
        "latitude": 29.9589,
        "longitude": -92.0332,
    },
    {
        "code": "WAHA",
        "parent_location_code": None,
        "location_kind": "POINT",
        "city": "Waha",
        "state_or_territory": "Texas",
        "continent": "North America",
        "latitude": 31.9493,
        "longitude": -103.6652,
    },
    {
        "code": "AECO",
        "parent_location_code": None,
        "location_kind": "POINT",
        "city": "Calgary",
        "state_or_territory": "Alberta",
        "continent": "North America",
        "latitude": 51.0447,
        "longitude": -114.0719,
    },
    {
        "code": "DAWN",
        "parent_location_code": None,
        "location_kind": "POINT",
        "city": "Dawn-Euphemia",
        "state_or_territory": "Ontario",
        "continent": "North America",
        "latitude": 42.7245,
        "longitude": -81.9055,
    },
    {
        "code": "PJM_WEST",
        "parent_location_code": None,
        "location_kind": "POINT",
        "city": "Pittsburgh",
        "state_or_territory": "Pennsylvania",
        "continent": "North America",
        "latitude": 40.4406,
        "longitude": -79.9959,
    },
    {
        "code": "ERCOT_NORTH",
        "parent_location_code": None,
        "location_kind": "POINT",
        "city": "Dallas",
        "state_or_territory": "Texas",
        "continent": "North America",
        "latitude": 32.7767,
        "longitude": -96.797,
    },
    {
        "code": "SP15",
        "parent_location_code": None,
        "location_kind": "POINT",
        "city": "Los Angeles",
        "state_or_territory": "California",
        "continent": "North America",
        "latitude": 34.0522,
        "longitude": -118.2437,
    },
]


def upgrade() -> None:
    op.add_column(
        "reference_locations",
        sa.Column("parent_location_code", sa.String(length=50), nullable=True),
    )
    op.add_column(
        "reference_locations",
        sa.Column("location_kind", sa.String(length=20), nullable=False, server_default="POINT"),
    )
    op.add_column(
        "reference_locations",
        sa.Column("city", sa.String(length=120), nullable=True),
    )
    op.add_column(
        "reference_locations",
        sa.Column("state_or_territory", sa.String(length=120), nullable=True),
    )
    op.add_column(
        "reference_locations",
        sa.Column("continent", sa.String(length=40), nullable=True),
    )
    op.add_column(
        "reference_locations",
        sa.Column("latitude", sa.Float(), nullable=True),
    )
    op.add_column(
        "reference_locations",
        sa.Column("longitude", sa.Float(), nullable=True),
    )

    op.create_index(
        "ix_reference_locations_parent_location_code",
        "reference_locations",
        ["parent_location_code"],
    )
    op.create_foreign_key(
        "fk_reference_locations_parent_location_code",
        "reference_locations",
        "reference_locations",
        ["parent_location_code"],
        ["code"],
    )

    reference_locations = sa.table(
        "reference_locations",
        sa.column("code", sa.String()),
        sa.column("location_type", sa.String()),
        sa.column("parent_location_code", sa.String()),
        sa.column("location_kind", sa.String()),
        sa.column("city", sa.String()),
        sa.column("state_or_territory", sa.String()),
        sa.column("continent", sa.String()),
        sa.column("latitude", sa.Float()),
        sa.column("longitude", sa.Float()),
    )

    op.execute(
        reference_locations.update().values(
            location_kind=sa.case(
                (
                    sa.func.upper(reference_locations.c.location_type) == "REGION",
                    "REGION",
                ),
                else_="POINT",
            )
        )
    )
    for row in LOCATION_BACKFILLS:
        code = row["code"]
        values = {key: value for key, value in row.items() if key != "code"}
        op.execute(
            reference_locations.update()
            .where(reference_locations.c.code == code)
            .values(**values)
        )

    op.alter_column("reference_locations", "location_kind", server_default=None)


def downgrade() -> None:
    op.drop_constraint(
        "fk_reference_locations_parent_location_code",
        "reference_locations",
        type_="foreignkey",
    )
    op.drop_index(
        "ix_reference_locations_parent_location_code",
        table_name="reference_locations",
    )
    op.drop_column("reference_locations", "longitude")
    op.drop_column("reference_locations", "latitude")
    op.drop_column("reference_locations", "continent")
    op.drop_column("reference_locations", "state_or_territory")
    op.drop_column("reference_locations", "city")
    op.drop_column("reference_locations", "location_kind")
    op.drop_column("reference_locations", "parent_location_code")
