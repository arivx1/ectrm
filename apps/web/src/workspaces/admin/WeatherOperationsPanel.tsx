            ...
            <div className="admin-weather-preview">
              <div className="shipment-card-meta">
                <span className="entity-chip entity-chip-soft">
                  Forecast {formatWeatherAgeHours(syncLocationByCode.get(selectedLocation.code)?.forecast_age_hours)}
                </span>
                <span className="entity-chip entity-chip-soft">
                  Observation {formatWeatherAgeHours(syncLocationByCode.get(selectedLocation.code)?.observation_age_hours)}
                </span>
                <span className="entity-chip entity-chip-soft">
                      ...
                      <div className="detail-list">
                        {forecastPeriods.map((period) => (
                          <article key={period.id} className="detail-row">
                            <span>{formatWeatherPeriodWindow(period.start_at, period.end_at)}</span>
                            <strong>{summarizeWeatherForecast(period)}</strong>
                          </article>
                        ))}
                      </div>
                    ) : (
                      ...
                      <div className="detail-list">
                        {observations.map((observation) => (
                          <article key={observation.id} className="detail-row">
                            ...
                            <strong>{summarizeWeatherObservation(observation)}</strong>
                          </article>
                        ))}
                      </div>
                    ) : (
