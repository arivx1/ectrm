...
| Operations | `domains/operations` | Confirmations, deliveries, shipments, gas schedule and nomination commitments, workflow items, actualization ledger evidence with inventory-deferred treatment, secondary cost lifecycle evidence, operational resources, document-intake work projections, tracking. |
| Settlement and accruals | `domains/settlement`, `domains/accruals`, settlement-adjacent operations services | Invoice/payment state, settlement posture, secondary cost preview inputs, accrual lots and entries, settlement exceptions. |
...
| Messages and collaboration | `routes/messages.py`, `domains/messages/services/workspace.py`, `domains/integrations/services/slack_messaging.py`, Slack read tools in `domains/assistant/services/tools.py` | Durable conversation/message objects, Slack sync and mirror posting, assistant read tools over mirrored Slack conversations, in-thread assistant provenance, and authenticated external messaging. Not a hidden business-write path. |
| Integrations | `routes/integrations.py`, `domains/integrations/services` | Admin-visible external SaaS connectors and authenticated adapters such as Attio CRM connection checks, deal-backed Attio CRM proposal/reconciliation, enrichment, and contact retention, Notion workspace and client-page search checks, Grain recording checks and CRM client recording search, Linear issue checks and CRM client issue search, Gmail inbox readiness checks, Slack messaging connection checks, read-only Nexus client/prospect engagement matching with participant-only Gmail searches and the Slack Messages mirror, and Anthropic admin API key status. Keep secrets in backend settings, return redacted runtime state, and route business writes through typed domain services. |
...
| Assistant and AI gateway | `domains/assistant`, `routes/assistant.py` | Prompt assembly, provider-routed OpenAI/Anthropic/Google runtime calls, provider-routed agent draft/self-update generation for supported structured-output providers, governed live read tools including Gmail inbox reads and Slack Messages mirror reads, managed agents, run traces, evals, action planning, action governance. |
