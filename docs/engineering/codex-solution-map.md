...
| Home view instances | `domains/home_views`, `routes/home_view_definitions.py`, Prompt Home workspace, assistant read tools, Home recipe registry | `apps.api.tests.test_home_view_definitions_api`; `apps.api.tests.test_home_view_recipes` for recipe changes; `apps.api.tests.test_assistant_tooling` when assistant Home tools change; focused assistant action/API tests plus `make api-assistant-evals` when prompt-created Home views or `create_home_view_instance` changes; focused web tests when Prompt Home rendering changes. |
