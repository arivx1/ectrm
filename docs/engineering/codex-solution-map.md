...
| Home view instances | `domains/home_views`, `routes/home_view_definitions.py`, assistant read tools in `domains/assistant/services/tools.py` | System Home template metadata, personal and shared Home definitions, card registry and card configuration value validation, shared lifecycle/admin inventory, assistant-readable Home catalog/visible-instance inspection, scope/audit fields, and reset behavior. Not live business data truth. |
...
| Home view instances | `domains/home_views`, `routes/home_view_definitions.py`, Prompt Home workspace, assistant read tools | `apps.api.tests.test_home_view_definitions_api`; `apps.api.tests.test_assistant_tooling` when assistant Home tools change; focused web tests when Prompt Home rendering changes. |
